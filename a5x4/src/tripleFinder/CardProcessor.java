...
public class CardProcessor implements CardProcessor_I {
    ...
    private final Map<Rank,Collection<Card>> cardsMap; /// = new HashMap<Rank,Collection<Card>>();
    ...
    public Object process(Card card) {
        ...
        if (rankCollection==null) {
            rankCollection = new ArrayList<>();
            cardsMap.put(currentRank, rankCollection);
        }
        rankCollection.add(card);
        /*
        //if Rank(key) of currentCard exists in hashmap 
        ...
        */
        ...
        if (rankCollection.size() >= 3) {
            return cardsMap.remove(currentRank);
        }
